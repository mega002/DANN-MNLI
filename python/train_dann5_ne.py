...
class modelClassifier:
    def __init__(self, seq_length):
        ...
        # entropy loss := total_loss = label_loss - lmda*H(domain_probs)
        self.total_cost = tf.add(self.model.pred_cost, tf.multiply(self.lmbda, self.model.domain_neg_entropy))
        tf.summary.scalar('total_loss', self.total_cost)

        # Perform GD with Adam
        domain_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='domain_classifier')
        non_domain_vars = [var for var in tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES) if var not in domain_vars]

        self.domain_optimizer = tf.train.AdamOptimizer(self.learning_rate, beta1=0.9, beta2=0.999).minimize(
                                                        self.model.domain_cost, var_list=domain_vars)
        self.total_optimizer = tf.train.AdamOptimizer(self.learning_rate, beta1=0.9, beta2=0.999).minimize(
                                                        self.total_cost, var_list=non_domain_vars)
    ...
    def train(self, training_labeled, dev_mat, dev_mismat, dev_snli):
        ...
        while True:
            random.shuffle(training_labeled)
            avg_cost = 0.
            total_batches = int(len(training_labeled) / self.batch_size)
            ...
            for i in range(total_batches):
                ...
                _, domain_cost = self.sess.run([self.domain_optimizer, self.model.domain_cost], feed_dict)
                _, total_cost, summary_results = self.sess.run([self.total_optimizer, self.total_cost, summary_op],
                                                               feed_dict)
                ...
                avg_cost += total_cost / (total_batches * self.batch_size)
            ...
            # p = float(self.steps) / 100000. , self.lmbda = 2. / (1. + np.exp(-10. * p)) - 1
...
if not test:
    classifier.train(training_labeled, dev_matched, dev_mismatched, dev_snli)
