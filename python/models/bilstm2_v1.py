...
class MyModel(object):
    def __init__(self, seq_length, emb_dim, hidden_dim, embeddings, emb_train):
        ...
        self.lmbda = tf.placeholder(tf.float32, [], name='lmda')
        ...
        with tf.variable_scope('domain_classifier'):
            ...
            h_ = flip_gradient(self.h, self.lmbda)
            ...
            self.W_domain_cl = tf.Variable(tf.random_normal([self.dim, 2], stddev=0.1), name='W_domain_cl')
            self.b_domain_cl = tf.Variable(tf.random_normal([2], stddev=0.1), name='b_domain_cl')

            # MLP layer
            h_domain_mlp = tf.nn.relu(tf.matmul(h_, self.W_domain_mlp) + self.b_domain_mlp)

            # Dropout applied to classifier
            h_domain_drop = tf.nn.dropout(h_domain_mlp, self.keep_rate_ph)

            # Get prediction
            self.domain_logits = tf.matmul(h_domain_drop, self.W_domain_cl) + self.b_domain_cl
        ...
        self.total_cost = self.pred_cost + self.domain_cost
