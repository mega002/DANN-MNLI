...
from models.bilstm5_1ul import *
...
indices_to_words, word_indices = sentences_to_padded_index_sequences([training_labeled, training_unlabeled, dev_snli,
                                                                      dev_matched, dev_mismatched, test_snli])
...
class modelClassifier:
    ...
    def get_minibatch(self, labeled_samples, unlabeled_samples, idx):
        ...
        s_premise_vectors, s_hypothesis_vectors, s_labels, s_genres, s_domains = \
                self.get_minibatch_samples(labeled_samples, indices)
        t_premise_vectors, t_hypothesis_vectors, t_labels, t_genres, t_domains = \
                self.get_minibatch_samples(unlabeled_samples, indices)
    ...
    def train(self, training_labeled, training_unlabeled, dev_mat, dev_mismat, dev_snli, acclog):
        ...
        unlabeled_batches = int(len(training_unlabeled) / self.batch_size)
...
if not test:
    ...
    classifier.train(training_labeled, training_unlabeled, dev_matched, dev_mismatched, dev_snli, acclog)
    acclog.close()
